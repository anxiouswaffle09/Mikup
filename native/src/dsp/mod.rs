...
/// Decode an entire WAV file to mono f32 samples at native sample rate.
pub fn decode_wav_to_mono(path: impl AsRef<Path>) -> Result<(Vec<f32>, u32), AudioDecodeError> {
    ...
    let track = format
        .default_track()
        .ok_or_else(|| AudioDecodeError::Probe("no audio track".to_string()))?;
    let track_id = track.id;
    let codec_params = track.codec_params.clone();
    let sample_rate = codec_params
        .sample_rate
        .ok_or_else(|| AudioDecodeError::Probe("missing sample rate".to_string()))?;
    ...
    loop {
        ...
        samples.extend(decode_to_normalized_mono(decoded));
    }
