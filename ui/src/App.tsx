  ...
  const runStage = async (stageIndex: number, force = false): Promise<void> => {
    ...
    if (stageIndex === DSP_STAGE_INDEX) {
      ...
      try {
        ...
        const dxPath = stems['DX'];
        const musicPath = stems['Music'];
        const effectsPath = stems['Effects'];

        if (!dxPath || !musicPath || !effectsPath) {
        ...
        dspStream.startStream(dxPath, musicPath, effectsPath);
      } catch (err: unknown) {
          setError(err instanceof Error ? err.message : String(err));
          setRunningStageIndex(null);
          setWorkflowMessage('DSP analysis failed. Check that Stage 1 stems exist and retry.');
